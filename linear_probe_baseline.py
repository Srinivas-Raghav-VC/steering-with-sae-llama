...
def evaluate_probe_steering(
    ...
    for prompt in tqdm(eval_prompts, desc="Evaluating"):
        ...
        baseline_lang = detect_language_simple(baseline)
        steered_lang = detect_language_simple(steered)
        
        # Check success based on mode
        if config.eval_mode == "shadow_hindi":
            is_success = (baseline_lang == "hindi" and steered_lang == "english")
        elif config.eval_mode == "shadow_english":
            is_success = (baseline_lang == "english" and steered_lang == "hindi")
        else:
            is_success = (steered_lang == target_lang.lower())
