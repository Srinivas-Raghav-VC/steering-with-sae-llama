...
class Pipeline:
    ...
    def _launch_ui(
        ...
        with gr.Blocks(theme=gr.themes.Soft()) as demo:
            gr.Markdown("# Hindi–English Feature Steering / Shadowing (Llama‑3.1‑8B)")

            gr.Markdown(
                """
            ##  How to Use This Interface

            This tool allows you to **steer** or **shadow** language generation in Llama-3.1-8B by manipulating neural features that distinguish Hindi from English.

            ###  **Target Language**
            - **Hindi**: Amplify Hindi features, suppress English features
            - **English**: Amplify English features, suppress Hindi features

            ###  **Mode**
            - **steer**: Full steering - amplify target language features AND suppress the other language
            - **shadow_hindi**: Shadow mode - suppress ONLY Hindi features (useful for forcing English output)
            - **shadow_english**: Shadow mode - suppress ONLY English features (useful for forcing Hindi output)

            ###  **Strength** (0.5 - 2.0)
            - **0.5-0.8**: Subtle effects, preserves original style
            - **1.0-1.5**: Moderate steering, good balance
            - **1.6-2.0**: Strong effects, may change style significantly

            ###  **Top-k Features** (1-10)
            - Number of most important features to manipulate per layer
            - Higher values = more features = stronger but potentially less precise effects

            ###  **Max New Tokens** (16-200)
            - Maximum length of generated text
            - Longer texts may show more pronounced steering effects

            ###  **Feature Toggles** (Optional)
            - Select specific features to enable/disable
            - Format: `L{layer}:{feature_id}` (e.g., L18:123 means feature 123 in layer 18)
            - Leave empty to use all top-k features automatically
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    # Feature toggles (flatten all features from all layers)
                    all_hi_feats = []
                    all_en_feats = []
                    ...
                    for L, feats in union_hi:
                        all_hi_feats.extend([pretty_label(L, f) for f in feats[:20]])

                    for L, feats in union_en:
                        all_en_feats.extend([pretty_label(L, f) for f in feats[:20]])

                    hi_check = gr.CheckboxGroup(
                        choices=all_hi_feats[:100],  # limit to 100 choices
                        label="Hindi Feature Toggles (Optional)",
                    )
                    en_check = gr.CheckboxGroup(
                        choices=all_en_feats[:100],  # limit to 100 choices
                        label="English Feature Toggles (Optional)",
                    )

                    custom_gains = gr.Textbox(
                        label="Custom per-feature gains (Optional)",
                        placeholder="e.g., L18:130821=+0.6; L19:117955=-0.4",
                        lines=2,
                    )
                    ...
                    - **English prompts**: "The weather today is", "What is the capital of France?", "Explain quantum physics"
                    - **Hindi prompts**: "आज का मौसम", "फ्रांस की राजधानी क्या है?", "क्वांटम भौतिकी समझाइए"
                    - **Mixed prompts**: "Hello नमस्ते", "I love भारत", "The weather आज बहुत अच्छा है"
                    """
                    )
            ...
            def run_ui(
                ...
                s = f"**Steered ({res['steered_lang']}):**\n{res['steered']}"
                b = f"**Baseline ({res['baseline_lang']}):**\n{res['baseline']}"
                i = (
                    f"Target: {target} | Mode: {mode} | Strength: {strength:.2f}\n\n"
                    f"Steered language: {res['steered_lang']} | "
                    f"Baseline language: {res['baseline_lang']}"
                )
                return s, b, i
            ...
            **Q: Why do steered and baseline outputs look the same?**
            - Try increasing the strength (1.2-1.6) or top-k features (5-7)
            - Some prompts may already be in the target language
            - Check the telemetry output in the console for feature counts

            **Q: How do I force Hindi output from English prompts?**
            - Use Target=Hindi, Mode=steer, Strength=1.5-2.0
            - Try prompts like "The weather today is" or "I love"

            **Q: How do I force English output from Hindi prompts?**
            - Use Target=English, Mode=shadow_hindi, Strength=1.2-1.6
            - Try prompts like "आज का मौसम" or "मैं प्यार करता हूं"

            **Q: What do the feature toggles do?**
            - They let you manually select which specific features to manipulate
            - Format: L{layer}:{feature_id} (e.g., L18:123)
            - Leave empty to use automatic top-k selection

            **Q: Why is generation slow?**
            - The model is running on GPU with multi-layer steering
            - Longer prompts and more features = slower generation
            - This is normal for research-grade steering
            """
            )

        print("Launching UI (public link enabled)")
